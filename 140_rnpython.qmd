...
Installation and configuration instructions are in the [reticulate documentation](https://rstudio.github.io/reticulate/articles/python_dependencies.html).
...
```{python impor_tips_fmri, eval=isTRUE(getOption("dase_python_ready"))}
import pandas as pd
...
```{r accesing_py, eval=isTRUE(getOption("dase_python_ready"))}
f1 <- subset(py$fmri, region == "parietal")
```


```{python accesing_r, eval=isTRUE(getOption("dase_python_ready"))}
...
```{python pandas, eval=isTRUE(getOption("dase_python_ready"))}
import pandas as pd 
...
```{python, eval=isTRUE(getOption("dase_python_ready"))}
...
```{python matplotlib, eval=isTRUE(getOption("dase_python_ready"))}
import matplotlib.pyplot as plt
import numpy as np
...
```{r fig.width=7, fig.height=3, echo=FALSE, eval=isTRUE(getOption("dase_python_ready"))}
library(ggplot2)
...
-   [RStudio: A Single Home for R and Python](https://posit.co/resources/videos/rstudio-a-single-home-for-r-and-python/)

-   [R interface to Python](https://rstudio.github.io/reticulate/)

-   [Practical Interoperability with R and Python](https://posit.co/blog/practical-interoperability/)
...
-   [Arrays in R and Python (reticulate)](https://rstudio.github.io/reticulate/articles/arrays.html)
