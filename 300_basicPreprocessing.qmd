...
_Imputation_ consists of replacing missing values with estimates. Many algorithms cannot handle missing values directly; therefore, imputation methods are needed. We can use simple approaches such as replacing missing values with the mean or mode of the attribute. More elaborated approaches include:
...
kc1_tbl %>%
    group_by(Defective) %>%
    summarise(
      branch_count_mean = mean(BRANCH_COUNT, na.rm = TRUE),
      branch_count_min = min(BRANCH_COUNT, na.rm = TRUE),
      branch_count_max = max(BRANCH_COUNT, na.rm = TRUE),
      loc_total_mean = mean(LOC_TOTAL, na.rm = TRUE),
      loc_total_min = min(LOC_TOTAL, na.rm = TRUE),
      loc_total_max = max(LOC_TOTAL, na.rm = TRUE)
    )
```
