...
## EDA Checklist for Software Engineering Data

Before modeling, verify at least:

1. Record counts and unique keys (files/modules/issues/commits)
2. Missing values per variable
3. Duplicated rows or duplicated IDs
4. Class imbalance (for defect labels)
5. Suspicious values (negative effort, impossible dates, zero size with non-zero effort)
6. Correlated predictors and potential redundancy
7. Data leakage risks (features created after the prediction time)
8. Temporal consistency (training data should precede test data)
...
```{r SDPlotExample, fig.cap="Plot example of the area within 2 and 4SD of the mean respectively", tidy=TRUE}
...
### Basic data quality checks (Telecom1)

```{r telecomQualityChecks, tidy=TRUE}
...
1. train on older projects/releases,
2. test on newer projects/releases.

```{r telecomTemporalSplit, tidy=TRUE}
# Temporal split by order: first 70% for training, last 30% for testing.
# Use this only when the rows are already ordered chronologically.
n <- nrow(telecom1)
cutoff <- floor(0.70 * n)
...
summary(telecom_test$effort)
```

If your dataset includes explicit dates (for example commit date or release date), sort by that field first and then split.
...
```{r chinaQualityChecks, tidy=TRUE}
dim(china)
colSums(is.na(china))
sum(duplicated(china))
```
