...
If you do not want to include vignettes you may include any number of R Markdown documents. 
...
1. Usually external files are placed in the dir ins/extdata. We may place the data there. 
2. Perform a first check of the package (click the button or devtools::check)
	-- Warning about the license --> correct, for instance, GPL-3
3. Usually, the external files are not uploaded to Github, specially their size is too big.
...
5. For the sake of example we copy and paste the files albretch.csv and bailey.csv in the folder inst/extdata
...
We will create a script in a new data_raw folder with `usethis::use_data_raw(name = "aandb")` (Give it the name that you like)
...
12. Create the file data.R in the R/ folder
More > Document    or devtools::document()

13. Delete the NAMESPACE file and repeat devtools::document() (NAMESPACE is overwritten)

14. Add the the following content (change as appropriate) to the file data.R 
