...
net.results <- compute(net_eff, testdata.norm[,2]) #Run them through the neural network
...
#set.seed(1234)
#splitIndex <- createDataPartition(kc1[, Defective], p = .75, list = FALSE, times = 1)
#kc1.train2 <- kc1[ splitIndex,]
#kc1.test2  <- kc1[-splitIndex,]
```
...
# get predictions on your testing data
predictions <- predict(object=objModel, kc1.test[,-22])


# library(pROC)
# auc <- roc(kc1.test[, 22], predictions)
# print(auc$auc)

postResample(pred=predictions, obs=kc1.test[,22])
