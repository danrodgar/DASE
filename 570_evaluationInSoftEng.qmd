...
- **Objective**: evaluate effort-estimation and defect-prediction models with SE-specific measures.
- **Primary metrics**: MAR/gMAR, SA, MARP0 variants, and standard ML metrics where applicable.
- **Common pitfalls**: using biased metrics alone (for example MMRE), weak baselines, and missing uncertainty reporting.

## Recommended reporting checklist

For reproducible model evaluation in software engineering, report:

1. data split strategy (including temporal aspects)
2. baseline model(s) and random-guessing comparator
3. main metric values and uncertainty intervals
4. metric limitations and rationale for chosen primary metric
5. subgroup or project-wise error analysis when possible
6. inter-rater agreement when labels come from human annotation

For classification-specific decisions, also include probability calibration and
cost-sensitive threshold analysis (see the evaluation chapters on
classification and graphical evaluation). For multi-model studies, report
statistical comparisons with multiplicity control and effect sizes.
...
There are several measures typically used in software engineering. In particular for effort estimation, the following metrics are extensively used in addition or instead of statistical measures.
  ...
  - *Mean Magnitude of the Relative Error (MMRE)*: this measure has been criticized many times as a biased measure 
